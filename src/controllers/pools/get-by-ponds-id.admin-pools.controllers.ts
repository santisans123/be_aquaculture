...
async function getPoolsByPondsIdAdmin(req: Request, res: Response) {
  ...
  const pools = await Pools.find({ pondsId: req.params.pondsId, ...NOT_ARCHIVED });
  if (!pools) {
    return res.status(404).send(
      message({
        statusCode: 404,
